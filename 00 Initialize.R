...
# required_packages = c('geneorama', 'data.table', 'ggplot2', 'zoo', 'dlm')
required_packages = c('geneorama', 'data.table', 'ggplot2')
## Packages installed on this computer
installed_pacakges =  .packages(all.available = TRUE)

## Try and install any missing packages
for(pkg in c(required_packages, special_packages)){
